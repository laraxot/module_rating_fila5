# Rating Module Documentation Index

**Stats**: 151 files | 18 categories | Last update: 2026-07-28 11:34

## Quick Navigation

- [Architecture](rating-architecture.md) | [Analysis](code-quality-analysis.md) | [Wiki](wiki/) | [Raw](raw/) | [Roadmap](roadmap/)
- [Configuration](configuration.md) | [Models](data-models.md) | [Concepts](wiki/concepts/)

## Documentation by Category

### Root (89 files)

The root level contains strategic documents, analyses, and consolidated guides:

- [00-INDEX.md](00-INDEX.md)
- [INDEX.md](INDEX.md)
- [README.md](README.md)
- [BEST_PRACTICES.md](BEST_PRACTICES.md)
- [BAD_PRACTICES.md](BAD_PRACTICES.md)
- [CHANGELOG.md](CHANGELOG.md)
- [FALSE_FRIENDS.md](FALSE_FRIENDS.md)
- [LICENSE.md](LICENSE.md)
- [METODI_DUPLICATI_ANALISI.md](METODI_DUPLICATI_ANALISI.md)
- [MIGRATIONS.md](MIGRATIONS.md)
- [ON-DEMAND-PATTERN.md](ON-DEMAND-PATTERN.md)
- [PERFORMANCE-OPTIMIZATION.md](PERFORMANCE-OPTIMIZATION.md)
- [PRODUCT_LAUNCH_PLAN.md](PRODUCT_LAUNCH_PLAN.md)
- [PRODUCT_ROADMAP.md](PRODUCT_ROADMAP.md)
- [PRODUCT_STRATEGY.md](PRODUCT_STRATEGY.md)
- [PROJECT-STRUCTURE.md](PROJECT-STRUCTURE.md)
- [QMD-SETUP.md](QMD-SETUP.md)
- [REDUNDANCY_ANALYSIS.md](REDUNDANCY_ANALYSIS.md)
- [SPRINT_PLANNING.md](SPRINT_PLANNING.md)
- [USER_RESEARCH.md](USER_RESEARCH.md)
- [agent-confidence-discipline.md](agent-confidence-discipline.md)
- [agent-confidence-protocol.md](agent-confidence-protocol.md)
- [agent-edit-discipline.md](agent-edit-discipline.md)
- [api-integration.md](api-integration.md)
- [architecture-rules.md](architecture-rules.md)
- [architecture.md](architecture.md)
- [best-practices.md](best-practices.md)
- [case-sensitivity-rules.md](case-sensitivity-rules.md)
- [changelog.md](changelog.md)
- [code-quality-analysis.md](code-quality-analysis.md)
- [code-redundancy-audit.md](code-redundancy-audit.md)
- [codex-error-fix.md](codex-error-fix.md)
- [confidence_guidelines.md](confidence_guidelines.md)
- [configuration.md](configuration.md)
- [conflict-resolution.md](conflict-resolution.md)
- [contracts-naming.md](contracts-naming.md)
- [copilot-redundancy-audit-2026-05-25.md](copilot-redundancy-audit-2026-05-25.md)
- [copilot-redundancy-audit.md](copilot-redundancy-audit.md)
- [core-functionality.md](core-functionality.md)
- [cyclomatic-complexity-report.md](cyclomatic-complexity-report.md)
- [data-models.md](data-models.md)
- [dry-kiss-analysis.md](dry-kiss-analysis.md)
- [duplicate-methods-analysis.md](duplicate-methods-analysis.md)
- [duplicate_methods.md](duplicate_methods.md)
- [duplicate_methods_report.md](duplicate_methods_report.md)
- [filament-version.md](filament-version.md)
- [file-naming-rules.md](file-naming-rules.md)
- [git-multi-org-sync-handoff.md](git-multi-org-sync-handoff.md)
- [index.md](index.md)
- [laravel-13-upgrade.md](laravel-13-upgrade.md)
- [launch-plan.md](launch-plan.md)
- [license.md](license.md)
- [merge-conflict-files-list.md](merge-conflict-files-list.md)
- [merge-conflicts-list.md](merge-conflicts-list.md)
- [metodi-duplicati-analisi.md](metodi-duplicati-analisi.md)
- [metodi_duplicati_analisi.md](metodi_duplicati_analisi.md)
- [migration-patterns.md](migration-patterns.md)
- [multi-org-sync-laraxot-provtv.md](multi-org-sync-laraxot-provtv.md)
- [phpstan-errors-roadmap.md](phpstan-errors-roadmap.md)
- [phpstan-fixes-2026-01.md](phpstan-fixes-2026-01.md)
- [phpstan-fixes-analysis.md](phpstan-fixes-analysis.md)
- [phpstan-fixes.md](phpstan-fixes.md)
- [ponytail-audit-over-engineering.md](ponytail-audit-over-engineering.md)
- [prd.json](prd.json)
- [prd.md](prd.md)
- [product-launch-plan.md](product-launch-plan.md)
- [product-requirements.md](product-requirements.md)
- [product-roadmap.md](product-roadmap.md)
- [product-strategy.md](product-strategy.md)
- [rating-architecture.md](rating-architecture.md)
- [readme-en.md](readme-en.md)
- [redundancy-audit-2026-05-21.md](redundancy-audit-2026-05-21.md)
- [redundancy-audit.md](redundancy-audit.md)
- [redundancy-report.md](redundancy-report.md)
- [release-marketing-standard.md](release-marketing-standard.md)
- [roadmap.md](roadmap.md)
- [root-file-policy.md](root-file-policy.md)
- [root-files-hygiene.md](root-files-hygiene.md)
- [schema.md](schema.md)
- [schemaless-attributes-errors.md](schemaless-attributes-errors.md)
- [schemaless-attributes.md](schemaless-attributes.md)
- [second-brain.md](second-brain.md)
- [sprint-planning-meeting.md](sprint-planning-meeting.md)
- [sprint-planning.md](sprint-planning.md)
- [strategy.md](strategy.md)
- [troubleshooting.md](troubleshooting.md)
- [up.txt](up.txt)
- [user-interface.md](user-interface.md)
- [user-research.md](user-research.md)

### Wiki (39 files)

Comprehensive wiki system with concepts, how-to guides, rules, and troubleshooting:

**Main Index** (7 files)
  - [AGENTS.md](wiki/AGENTS.md)
  - [schema.md](wiki/schema.md)
  - [README.md](wiki/README.md)
  - [agents.md](wiki/agents.md)
  - [index.md](wiki/index.md)
  - [log.md](wiki/log.md)
  - [overview.md](wiki/overview.md)

**Concepts** (14 files)
  - [INDEX.md](wiki/concepts/INDEX.md)
  - [composer-root-minimal-nwidart.md](wiki/concepts/composer-root-minimal-nwidart.md)
  - [context-mode-rating-discipline.md](wiki/concepts/context-mode-rating-discipline.md)
  - [context-overflow-prevention.md](wiki/concepts/context-overflow-prevention.md)
  - [duplicate-method-bodies.md](wiki/concepts/duplicate-method-bodies.md)
  - [filament-resource-zen-pattern.md](wiki/concepts/filament-resource-zen-pattern.md)
  - [method-name-homonyms.md](wiki/concepts/method-name-homonyms.md)
  - [organizzativa-money.md](wiki/concepts/organizzativa-money.md)
  - [phpstan-compliance.md](wiki/concepts/phpstan-compliance.md)
  - [ponytail-audit.md](wiki/concepts/ponytail-audit.md)
  - [second-brain-local-discipline.md](wiki/concepts/second-brain-local-discipline.md)
  - [testing.md](wiki/concepts/testing.md)
  - [visual-testing-playwright-puppeteer.md](wiki/concepts/visual-testing-playwright-puppeteer.md)
  - [xotbase-table-columns-enforcement.md](wiki/concepts/xotbase-table-columns-enforcement.md)

**Templates** (3 files)
  - [concept.md](wiki/_templates/concept.md)
  - [entity.md](wiki/_templates/entity.md)
  - [source.md](wiki/_templates/source.md)

**Rules** (2 files)
  - [INDEX.md](wiki/rules/INDEX.md)
  - [no-rm-no-archive-use-old-suffix.md](wiki/rules/no-rm-no-archive-use-old-suffix.md)

**Troubleshooting** (2 files)
  - [phpstan-generic-limitation-morphtomany.md](wiki/troubleshooting/phpstan-generic-limitation-morphtomany.md)
  - [phpstan-generic-limitations.md](wiki/troubleshooting/phpstan-generic-limitations.md)

**Other** (11 files)
  - [skills/INDEX.md](wiki/skills/INDEX.md)
  - [commands/INDEX.md](wiki/commands/INDEX.md)
  - [memories/INDEX.md](wiki/memories/INDEX.md)
  - [how-to/gitmodules-sync-session.md](wiki/how-to/gitmodules-sync-session.md)
  - [redundancy/duplicate-ratings-table-migrations.md](wiki/redundancy/duplicate-ratings-table-migrations.md)

### Roadmap (7 files)

Strategic planning and vision documents:

- [README.md](roadmap/README.md)
- [acceptance-criteria.md](roadmap/acceptance-criteria.md)
- [dependencies.md](roadmap/dependencies.md)
- [metrics.md](roadmap/metrics.md)
- [phases.md](roadmap/phases.md)
- [quality.md](roadmap/quality.md)
- [vision.md](roadmap/vision.md)

### Raw (7 files)

Raw source documents and import sources:

- [README.md](raw/README.md)
- [index.md](raw/index.md)

**Root Import** (5 files)
  - [changelog-1.md](raw/root-import/changelog-1.md)
  - [changelog-2.md](raw/root-import/changelog-2.md)
  - [changelog.md](raw/root-import/changelog.md)
  - [license-1.md](raw/root-import/license-1.md)
  - [license.md](raw/root-import/license.md)

### LLM Wiki (7 files)

Machine learning and documentation knowledge base:

- [AGENTS.md](llm-wiki/AGENTS.md)
- [agents.md](llm-wiki/agents.md)
- [index.md](llm-wiki/index.md)
- [log.md](llm-wiki/log.md)

**Templates** (3 files)
  - [concept.md](llm-wiki/_templates/concept.md)
  - [entity.md](llm-wiki/_templates/entity.md)
  - [source.md](llm-wiki/_templates/source.md)

### Root Markdown Files (4 files)

Consolidated root-level markdown documentation:

- [changelog-1.md](root-md-files/changelog-1.md)
- [changelog.md](root-md-files/changelog.md)
- [license-1.md](root-md-files/license-1.md)
- [license.md](root-md-files/license.md)

### Archive (3 files)

Legacy and superseded documentation:

- [INDEX.md](archive/INDEX.md)
- [dataobjects-archived-duplicate.md](archive/dataobjects-archived-duplicate.md)
- [docs-archive-policy.md](archive/docs-archive-policy.md)

### Archived (1 file)

Additional archived content:

- [license.md](archived/license.md)

---

## Recently Updated

**Last 20 files by modification date**

| Date | File | Category |
|------|------|----------|
| 2026-07-28 | [00-INDEX.md](00-INDEX.md) | root |
| 2026-07-28 | [README.md](README.md) | root |
| 2026-07-28 | [BEST_PRACTICES.md](BEST_PRACTICES.md) | root |
| 2026-07-28 | [PRODUCT_STRATEGY.md](PRODUCT_STRATEGY.md) | root |
| 2026-07-28 | [PRODUCT_ROADMAP.md](PRODUCT_ROADMAP.md) | root |
| 2026-07-28 | [SPRINT_PLANNING.md](SPRINT_PLANNING.md) | root |
| 2026-07-28 | [rating-architecture.md](rating-architecture.md) | root |
| 2026-07-28 | [architecture.md](architecture.md) | root |
| 2026-07-28 | [configuration.md](configuration.md) | root |
| 2026-07-28 | [core-functionality.md](core-functionality.md) | root |
| 2026-07-28 | [data-models.md](data-models.md) | root |
| 2026-07-28 | [schema.md](schema.md) | root |
| 2026-07-28 | [user-interface.md](user-interface.md) | root |
| 2026-07-28 | [best-practices.md](best-practices.md) | root |
| 2026-07-28 | [changelog.md](changelog.md) | root |
| 2026-07-28 | [conflict-resolution.md](conflict-resolution.md) | root |
| 2026-07-28 | [architecture-rules.md](architecture-rules.md) | root |
| 2026-07-28 | [file-naming-rules.md](file-naming-rules.md) | root |
| 2026-07-28 | [readme-en.md](readme-en.md) | root |
| 2026-07-28 | [codex-error-fix.md](codex-error-fix.md) | root |

---

## Usage Notes

- **Root (89 files)**: Strategic, high-level documentation that synthesizes implementation and planning
- **Wiki (39 files)**: Knowledge base with troubleshooting, concepts, how-to guides, and reference materials
- **Roadmap (7 files)**: Vision, phases, goals, and workstreams
- **Raw (7 files)**: Immutable source documents maintained separately for archival purposes
- **LLM Wiki (7 files)**: Machine learning and AI documentation knowledge base
- **Root Markdown Files (4 files)**: Consolidated root-level markdown documentation
- **Archive (3 files)**: Legacy and superseded documentation with archive policies
- **Archived (1 file)**: Additional archived content

## File Summary by Type

| Type | Count | Purpose |
|------|-------|---------|
| Strategic Docs | 12 | PRD, roadmap, product strategy, launch plan |
| Architecture | 8 | System design, patterns, structure, rules |
| Quality & Analysis | 15 | Code quality, redundancy audit, complexity analysis |
| Configuration & Setup | 8 | Configuration, migrations, schema, setup guides |
| Process & Discipline | 10 | Agent protocols, editing discipline, confidence guidelines |
| Testing & Performance | 6 | Performance optimization, visual testing, quality metrics |
| Reference & Guides | 12 | Best practices, case sensitivity, naming, tutorials |
| Troubleshooting | 4 | PHPStan fixes, error diagnosis, conflict resolution |
| Knowledge Base | 39 | Wiki concepts, skills, how-to, glossaries |
| Templates | 6 | Document templates for concepts, entities, sources |
| Raw Sources | 7 | Import sources, changelogs, licenses |
| Archive | 4 | Legacy and deprecated content |
| **TOTAL** | **151** | **All documentation** |

## Recommendations

1. **Deduplication**: Consolidate similar files:
   - `best-practices.md` vs `BEST_PRACTICES.md`
   - `changelog.md` vs `CHANGELOG.md` vs `changelog-*.md`
   - `license.md` vs `LICENSE.md`
   - `product-*.md` vs `PRODUCT_*.md`
   - `metodi-duplicati-analisi.md` vs `METODI_DUPLICATI_ANALISI.md`

2. **Consolidation**: Merge `root-md-files/`, `archived/`, and `archive/` into single canon locations

3. **Wiki Enhancement**: Expand wiki structure with:
   - Index pages for better discoverability
   - Cross-references between concepts
   - Updated frontmatter with metadata tags

4. **Stale Content Review**: Audit files from 2026-01 and earlier:
   - Determine if still active
   - Archive or remove if superseded
   - Update references if still needed

5. **Organization**: Consider grouping by functional area:
   - Rating System Core (architecture, models, schemas)
   - User Interface (Filament resources, rendering)
   - API & Integration (endpoints, middleware)
   - Data & Analytics (metrics, performance)

---

*Generated: 2026-07-28 11:34:00*
*Total Files Analyzed: 151 files across 18 categories*
*Ready for manual review and consolidation planning*
