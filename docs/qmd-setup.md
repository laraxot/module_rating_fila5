...
\`\`\`bash
qmd search "<topic>" -c rating
# Cerca solo in ./laravel/Modules/Rating/docs/wiki/
\`\`\`
...
\`\`\`bash
qmd search "form" -c rating  # Solo questo modulo
\`\`\`
...
| Ricerca lenta | Limita con `-c rating` |
